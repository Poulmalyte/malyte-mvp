...
export default function Step4Preview({ onComplete, onBack }: { onComplete: (data: any) => void, onBack: () => void }) {
  ...
  async function generatePreview() {
    ...
    try {
      ...
      setPlan(json.plan)
    } catch (e: any) { setError(e.message || 'Error generating preview') }
    setLoading(false)
  }
  ...
  if (error) return (
    <div>
      <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0F172A', margin: '0 0 20px', fontFamily: "'Satoshi', sans-serif" }}>Plan Preview</h2>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#F5F3FF', borderRadius: 10, border: '1px solid #DDD6FE', marginBottom: 20 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: '#94A3B8', margin: 0 }}>{plan?.demo_profile?.skin_type && `${plan.demo_profile.skin_type} skin · `}{plan?.demo_profile?.objectives?.join(', ')}</p>
        </div>
      </div>
      ...
      {plan?.products_to_introduce_next_week?.length > 0 && (
        <div style={{ padding: '12px 16px', background: '#F8FAFC', borderRadius: 10, border: '1px solid #E8EDF8', marginBottom: 16 }}>
      ...
      <div style={{ padding: '12px 16px', background: '#F0FDF4', borderRadius: 10, border: '1px solid #6EE7B7', marginBottom: 20 }}>
        <p style={{ fontSize: 12, color: '#065F46', margin: 0 }}>✓ This plan was generated in real-time using your products and your rules. Every customer gets a unique plan.</p>
      </div>
