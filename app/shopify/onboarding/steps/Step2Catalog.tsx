...
interface CatalogItem {
  ...
  title: string
  ai_tagged: boolean
  catalog_item_tags: Array<{ tag_type: string; tag_value: string; confidence_score: number; source: string }>
}
...
export default function Step2Catalog({ merchantId, hasInstallation, initialItems, onComplete, onBack }: { merchantId: string, hasInstallation: boolean, initialItems: CatalogItem[], onComplete: (data: any) => void, onBack: () => void }) {
  ...
  useEffect(() => { if (initialItems.length === 0 && hasInstallation) handleTagProducts() }, [])
  ...
  async function handleTagProducts() {
    ...
    try {
      ...
      if (res2.ok) { const json2 = await res2.json(); setItems(json2.items || []) }
    } catch (e: any) { setError(e.message || 'Error tagging products') }
    setTagging(false)
  }
  ...
  if (!hasInstallation) return (
    <div>
      ...
      <p style={{ fontSize: 14, color: '#64748B', margin: '0 0 20px' }}>Connect your Shopify store first to sync and tag your products.</p>
      <div style={{ padding: '16px', background: '#FEF3C7', borderRadius: 12, border: '1px solid #FDE68A', marginBottom: 20 }}>
        <p style={{ fontSize: 13, color: '#92400E', margin: 0 }}>No Shopify store connected. Go to Settings to connect your store first.</p>
      </div>
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={() => onComplete({ catalog_approved: true })} style={{ flex: 1, padding: '14px', borderRadius: 12, fontWeight: 700, fontSize: 14, background: '#7C5CFC', color: '#fff', border: 'none', cursor: 'pointer' }}>Skip for now</button>
      </div>
    </div>
  )
    ...
    <div>
      ...
      {tagResult && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 20 }}>
          {[{ label: 'Products found', value: tagResult.total, color: '#7C5CFC', bg: '#EDE9FE' }, { label: 'Tagged', value: tagResult.tagged, color: '#059669', bg: '#D1FDF3' }, { label: 'Need review', value: needsReview.length, color: '#D97706', bg: '#FEF3C7' }].map((kpi, i) => (
      ...
      {error && <div style={{ padding: '10px 14px', borderRadius: 8, marginBottom: 16, background: '#FEF2F2', border: '1px solid #FECACA', color: '#EF4444', fontSize: 13 }}>{error} <button onClick={handleTagProducts} style={{ marginLeft: 8, fontWeight: 700, color: '#EF4444', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontSize: 13 }}>Retry</button></div>}

      {items.length === 0 && (
        <div style={{ textAlign: 'center', padding: '32px 0' }}>
          <p style={{ color: '#94A3B8', fontSize: 14 }}>No products found.</p>
          <button onClick={handleTagProducts} style={{ marginTop: 12, padding: '10px 24px', borderRadius: 10, fontWeight: 700, fontSize: 13, background: '#7C5CFC', color: '#fff', border: 'none', cursor: 'pointer' }}>Sync & tag products</button>
        </div>
      )}
      ...
      {items.length > 0 && (
        <div style={{ padding: '12px 16px', background: '#F0FDF4', borderRadius: 10, border: '1px solid #6EE7B7', marginBottom: 20 }}>
          <p style={{ fontSize: 13, color: '#065F46', margin: 0 }}>✓ {highConfidence.length} products tagged with high confidence. You can refine tags anytime from the Products tab.</p>
        </div>
      )}

      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={onBack} style={{ flex: 1, padding: '14px', borderRadius: 12, fontWeight: 700, fontSize: 14, background: '#F8FAFC', color: '#64748B', border: '1px solid #E8EDF8', cursor: 'pointer' }}>Back</button>
        <button onClick={() => onComplete({ catalog_approved: true })} style={{ flex: 2, padding: '14px', borderRadius: 12, fontWeight: 700, fontSize: 14, background: '#7C5CFC', color: '#fff', border: 'none', cursor: 'pointer' }}>Approve & continue</button>
      </div>
    </div>
  )
}
