...
export default function OnboardingWizard({ merchant, merchantProfile, catalogItems, installation, userEmail, initialStep }: Props) {
  ...
  const [merchantData, setMerchantData] = useState({ seller_type: merchant?.seller_type || 'brand', category: merchant?.category || '', ...merchantProfile })
  const [catalogItemsState] = useState(catalogItems)

  async function saveStep(step: number, data: any) {
    setSaving(true)
    try {
      const res = await fetch('/api/shopify/save-onboarding', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ step, data }) })
      const json = await res.json()
      if (!res.ok) { console.error('Save step error:', json.error); return false }
      return true
    } catch (err) { console.error('Save step error:', err); return false }
  ...
  async function handleStep1Complete(data: any) {
    ...
    if (ok) { setCurrentStep(2); window.history.pushState({}, '', '/shopify/onboarding?step=2') }
  }

  async function handleStep2Complete(data: any) {
    const ok = await saveStep(2, data)
    if (ok) { setCurrentStep(3); window.history.pushState({}, '', '/shopify/onboarding?step=3') }
  }

  async function handleStep3Complete(data: any) {
    setMerchantData((prev: any) => ({ ...prev, ...data }))
    const ok = await saveStep(3, data)
    if (ok) { setCurrentStep(4); window.history.pushState({}, '', '/shopify/onboarding?step=4') }
  }

  async function handleStep4Complete(data: any) {
    const ok = await saveStep(4, data)
    if (ok) { setCurrentStep(5); window.history.pushState({}, '', '/shopify/onboarding?step=5') }
  }

  async function handleStep5Complete() {
    const ok = await saveStep(5, { seller_type: merchantData.seller_type, category: merchantData.category })
    if (ok) { router.push('/shopify') }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F7FA', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E8EDF8', padding: '0 24px' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0 16px' }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: '#94A3B8' }}>Step {currentStep} of {STEPS.length}</span>
          </div>
          <div style={{ height: 3, background: '#E8EDF8', borderRadius: 100, marginBottom: 0 }}>
            <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg, #7C5CFC, #06B6D4)', borderRadius: 100, transition: 'width 0.4s ease' }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0 0' }}>
            {STEPS.map(s => (
              <span key={s.number} style={{ fontSize: 10, fontWeight: 700, color: s.number === currentStep ? '#7C5CFC' : s.number < currentStep ? '#059669' : '#CBD5E1', letterSpacing: '0.04em' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 24px 64px' }}>
        <div style={{ background: '#FFFFFF', borderRadius: 20, border: '1px solid #E8EDF8', padding: '32px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
          {saving && (
            <div style={{ position: 'fixed', top: 20, right: 20, background: '#0F172A', color: '#fff', padding: '8px 16px', borderRadius: 100, fontSize: 12, fontWeight: 600, zIndex: 100 }}>Saving…</div>
          )}
