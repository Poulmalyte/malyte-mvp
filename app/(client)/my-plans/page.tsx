...
export default async function MyPlansPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#F5F7FA', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column' }}>
      <style>{`
        .myplans-header-inner { max-width: 720px; margin: 0 auto; }
        .myplans-header-top { display: flex; justify-content: space-between; align-items: center; padding: 16px 0; }
        .myplans-body { flex: 1; max-width: 720px; margin: 0 auto; width: 100%; padding: 24px 24px 48px; }
        .myplans-card { background: #FFFFFF; border-radius: 14px; padding: 20px 24px; border: 1px solid #E8EDF8; }
        .myplans-card-inner { display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap; margin-bottom: 12px; }

        @media (max-width: 640px) {
          .myplans-header-inner { padding: 0 4px; }
          .myplans-body { padding: 16px 16px 48px; }
          .myplans-card { padding: 16px; }
          .myplans-card-inner { flex-direction: column; align-items: flex-start; gap: 12px; }
          .myplans-open-btn { width: 100%; text-align: center; }
        }
      `}</style>

      {/* HEADER */}
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E8EDF8', padding: '0 24px' }}>
        <div className="myplans-header-inner">
          <div className="myplans-header-top">
        ...
        {!purchases || purchases.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {purchases.map((purchase: any) => {
              ...
              return (
                <div key={purchase.id} className="myplans-card">
                  <div className="myplans-card-inner">
                    <div>
                    ...
                    <Link href={`/my-plans/${purchase.id}/plan`} className="myplans-open-btn" style={{
