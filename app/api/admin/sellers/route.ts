...
export async function GET(request: NextRequest) {
  ...
  if (search) {
    query = query.or(`shop_name.ilike.%${search}%,shopify_domain.ilike.%${search}%`)
  }
  ...
  const [mcRes, profilesRes, checkinsRes, ordersRes, lastActivityRes] = await Promise.all([
    // merchant_customers per contare clienti unici per merchant
    supabase.from('merchant_customers')
      .select('merchant_id, customer_id')
      .in('merchant_id', merchantIds),
    // quiz completions = customer_profiles
    ...
    supabase.from('attributed_orders')
      .select('merchant_id, order_value')
      .in('merchant_id', merchantIds),
    supabase.from('customer_profiles')
      .select('merchant_id, created_at')
      .in('merchant_id', merchantIds)
  ...
  for (const p of profilesRes.data || []) {
    quizByMerchant[p.merchant_id] = (quizByMerchant[p.merchant_id] || 0) + 1
  }

  const checkinsByMerchant: Record<string, number> = {}
  for (const c of checkinsRes.data || []) {
    checkinsByMerchant[c.merchant_id] = (checkinsByMerchant[c.merchant_id] || 0) + 1
  }
