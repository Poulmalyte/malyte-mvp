...
const HOW_IT_WORKS = [
  ...
  },
  {
    ...
    desc: 'At the end of each week, you complete a quick check-in. The AI uses your progress and the expert\'s methodology to adapt the next week\'s plan.',
      ...
      <div style={{ minHeight: '100vh', background: '#FFFFFF', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div className="hero-padding" style={{ background: '#F5F7FA', borderBottom: '1px solid #E8EDF8', padding: '48px 20px 36px' }}>
          <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
            <h1 className="hero-title" style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 'clamp(28px, 5vw, 46px)', color: '#0F172A', marginBottom: 12, lineHeight: 1.15 }}>
              Find the perfect wellness plan<br />
              <span style={{ color: '#7C5CFC' }}>tailored just for you</span>
            </h1>
            <p className="hero-subtitle" style={{ color: '#64748B', fontSize: 16, marginBottom: 28, lineHeight: 1.6 }}>
        ...
        <div style={{ borderBottom: '1px solid #E8EDF8', background: '#FFFFFF', overflowX: 'auto', WebkitOverflowScrolling: 'touch' } as any}>
          <div className="cat-tabs" style={{ display: 'flex', gap: 0, padding: '0 16px', minWidth: 'max-content', margin: '0 auto', maxWidth: 1100, justifyContent: 'center' }}>
            {CATEGORIES.map(cat => {
                  ...
                  <Link key={product.id} href={`/product/${product.id}`} style={{ textDecoration: 'none' }}>
                    <div style={{
                      ...
                      <div style={{ height: 130, background: gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                        ...
                        {badge && (
                          <div style={{ position: 'absolute', top: 8, left: 8, background: badge.bg, color: badge.color, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 4 }}>
                            {badge.label}
                          </div>
                        )}
                        {product.duration_months && (
                          <div style={{ position: 'absolute', bottom: 8, right: 8, background: 'rgba(255,255,255,0.92)', color: '#334155', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4 }}>
                      ...
                      <div style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                          <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'linear-gradient(135deg, #7C5CFC, #6385FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                        ...
                        <p style={{ fontSize: 13, fontWeight: 500, color: '#0F172A', lineHeight: 1.4, marginBottom: 8, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: 36 }}>
                          {product.title}
                        </p>
                        <div style={{ borderTop: '1px solid #F1F5F9', paddingTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <span style={{ fontSize: 15, fontWeight: 700, color: '#0F172A' }}>€{Number(product.price).toFixed(0)}</span>
                        </div>
                      </div>
                    </div>
                  </Link>
        ...
        {/* HOW IT WORKS */}
        <div style={{ background: '#F5F7FA', borderTop: '1px solid #E8EDF8', borderBottom: '1px solid #E8EDF8', padding: '56px 16px' }}>
          <div style={{ maxWidth: 860, margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: '#7C5CFC', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: 10 }}>
                How it works
              </span>
              <h2 style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 'clamp(22px, 4vw, 34px)', color: '#0F172A', marginBottom: 14, lineHeight: 1.2 }}>
            ...
            <div className="how-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 16, marginBottom: 40 }}>
              {HOW_IT_WORKS.map((step, i) => (
                <div className="how-card" key={i} style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid #E8EDF8', padding: '24px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: i % 2 === 0 ? '#EDE9FE' : '#D1FDF3', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>
                    {step.emoji}
                  </div>
                  <div>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#7C5CFC', letterSpacing: '0.08em' }}>STEP {step.n}</span>
                    <h3 style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 700, fontSize: 16, color: '#0F172A', margin: '4px 0 8px', lineHeight: 1.3 }}>
                      {step.title}
                    </h3>
                    <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.7, marginBottom: 10 }}>
                      {step.desc}
                    </p>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 100, padding: '3px 10px' }}>
            ...
            {/* TRUST BAR */}
            <div style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid #E8EDF8', padding: '24px', marginBottom: 40 }}>
              <p style={{ textAlign: 'center', fontSize: 12, fontWeight: 700, color: '#94A3B8', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 20 }}>
                Why people choose Malyte
              </p>
              <div className="trust-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, maxWidth: 480, margin: '0 auto' }}>
                {TRUST_POINTS.map((point, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F5F7FA', borderRadius: 10, padding: '12px 14px' }}>
            ...
            {/* BIG CTA */}
            <div className="cta-box" style={{ background: 'linear-gradient(135deg, #7C5CFC 0%, #6385FF 100%)', borderRadius: 20, padding: '40px 28px', color: '#fff', textAlign: 'center' }}>
              <h2 className="cta-title" style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 'clamp(20px, 3vw, 30px)', marginBottom: 10, lineHeight: 1.2 }}>
              ...
              <Link href="/signup" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'inline-block', background: '#FFFFFF', color: '#7C5CFC', fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 15, padding: '12px 28px', borderRadius: 100, cursor: 'pointer' }}>
                  Get your personalized plan →
                </div>
              </Link>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', marginTop: 14 }}>
        ...
        {/* FOOTER */}
        <div style={{ padding: '24px 16px', textAlign: 'center', background: '#FFFFFF', borderTop: '1px solid #E8EDF8' }}>
