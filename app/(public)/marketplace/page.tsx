...
const HOW_IT_WORKS = [
  ...
  },
  {
    ...
    desc: 'At the end of each week, you complete a quick check-in. The AI uses your progress and the expert\'s methodology to adapt the next week\'s plan, so it always fits where you are now.',
...
export default async function MarketplacePage({
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FFFFFF', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* HERO */}
      <div style={{ background: '#F5F7FA', borderBottom: '1px solid #E8EDF8', padding: '56px 24px 40px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
          <Link href="/" style={{ textDecoration: 'none' }}>
            <span style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 22, color: '#0F172A', display: 'block', marginBottom: 24 }}>
              malyte<span style={{ color: '#7C5CFC' }}>.</span>
            </span>
          </Link>
          <h1 style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 'clamp(30px, 5vw, 46px)', color: '#0F172A', marginBottom: 12, lineHeight: 1.15 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
            {['Lose weight', 'Build muscle', 'Clear skin', 'Eat better', 'Mental wellness'].map(s => (
              <span key={s} style={{ fontSize: 13, color: '#7C5CFC', background: '#EDE9FE', padding: '5px 14px', borderRadius: 100, cursor: 'pointer', fontWeight: 500 }}>
                {s}
              </span>
            ))}
          </div>
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <Link key={cat.id} href={cat.id ? `/marketplace?goal=${cat.id}` : '/marketplace'} style={{ textDecoration: 'none' }}>
                <div style={{
                  padding: '16px 22px', display: 'flex', alignItems: 'center', gap: 7,
              ...
              return (
                <Link key={product.id} href={`/product/${product.id}`} style={{ textDecoration: 'none' }}>
                  <div style={{
                    ...
                    onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-3px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 8px 28px rgba(0,0,0,0.09)' }}
                    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLDivElement).style.boxShadow = 'none' }}
                  >
                    <div style={{ height: 148, background: gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                      <span style={{ fontSize: 52 }}>{emoji}</span>
                      {badge && (
                        <div style={{ position: 'absolute', top: 10, left: 10, background: badge.bg, color: badge.color, fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 4 }}>
                          {badge.label}
                        </div>
                      )}
                      {product.duration_months && (
                        <div style={{ position: 'absolute', bottom: 10, right: 10, background: 'rgba(255,255,255,0.92)', color: '#334155', fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 4 }}>
                    ...
                    <div style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 9 }}>
                        <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg, #7C5CFC, #6385FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                          {getInitials(product.expert_name)}
                        </div>
                        <span style={{ fontSize: 12, color: '#64748B', fontWeight: 500 }}>{username}</span>
                        <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 600, background: '#EDE9FE', color: '#7C5CFC', padding: '2px 7px', borderRadius: 4 }}>
                          {product.expert_category}
                        </span>
                      </div>
                      <p style={{ fontSize: 13, fontWeight: 500, color: '#0F172A', lineHeight: 1.45, marginBottom: 9, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: 38 }}>
                        {product.title}
                      </p>
                      {product.client_count > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 9 }}>
                          <span style={{ color: '#F59E0B', fontSize: 12 }}>★</span>
                          <span style={{ fontSize: 12, fontWeight: 600, color: '#0F172A' }}>{(4.5 + Math.min(product.client_count * 0.05, 0.5)).toFixed(1)}</span>
                          <span style={{ fontSize: 12, color: '#94A3B8' }}>({product.client_count})</span>
                        </div>
                      )}
                      <div style={{ borderTop: '1px solid #F1F5F9', paddingTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ fontSize: 16, fontWeight: 700, color: '#0F172A' }}>€{Number(product.price).toFixed(0)}</span>
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      {/* HOW IT WORKS */}
      <div style={{ background: '#F5F7FA', borderTop: '1px solid #E8EDF8', borderBottom: '1px solid #E8EDF8', padding: '72px 24px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#7C5CFC', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: 12 }}>
              How it works
            </span>
            <h2 style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 'clamp(26px, 4vw, 38px)', color: '#0F172A', marginBottom: 16, lineHeight: 1.2 }}>
            ...
            <p style={{ fontSize: 16, color: '#64748B', maxWidth: 540, margin: '0 auto', lineHeight: 1.7 }}>
              Unlike generic apps that give everyone the same plan, Malyte takes a real expert's methodology and uses AI to build something that fits only you.
            </p>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: 20, marginBottom: 56 }}>
            {HOW_IT_WORKS.map((step, i) => (
              <div key={i} style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid #E8EDF8', padding: '28px 32px', display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, flexShrink: 0, background: i % 2 === 0 ? '#EDE9FE' : '#D1FDF3', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
                  {step.emoji}
                </div>
                <div>
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#7C5CFC', letterSpacing: '0.08em' }}>STEP {step.n}</span>
                  <h3 style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 700, fontSize: 17, color: '#0F172A', margin: '4px 0 10px', lineHeight: 1.3 }}>
                    {step.title}
                  </h3>
                  <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.7, marginBottom: 12 }}>
                    {step.desc}
                  </p>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 100, padding: '4px 12px' }}>
                    <span style={{ color: '#059669', fontSize: 12 }}>✓</span>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#059669' }}>{step.proof}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          {/* TRUST BAR */}
          <div style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid #E8EDF8', padding: '32px 36px', marginBottom: 48 }}>
          ...
          {/* BIG CTA */}
          <div style={{ background: 'linear-gradient(135deg, #7C5CFC 0%, #6385FF 100%)', borderRadius: 20, padding: '48px 40px', color: '#fff', textAlign: 'center' }}>
            <h2 style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 'clamp(22px, 3vw, 32px)', marginBottom: 12, lineHeight: 1.2 }}>
            ...
            <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.8)', marginBottom: 28, lineHeight: 1.6 }}>
              ...
              Built in a few seconds by an expert who knows what works.
            </p>
            <Link href="/signup" style={{ textDecoration: 'none' }}>
              <div style={{ display: 'inline-block', background: '#FFFFFF', color: '#7C5CFC', fontFamily: 'Satoshi, sans-serif', fontWeight: 800, fontSize: 16, padding: '14px 36px', borderRadius: 100, cursor: 'pointer' }}>
                Get your personalized plan →
              </div>
            </Link>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', marginTop: 16 }}>
              One-time purchase or monthly plan — you choose what works for you.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* FOOTER */}
      <div style={{ padding: '28px 24px', textAlign: 'center', background: '#FFFFFF', borderTop: '1px solid #E8EDF8' }}>
